...
hdu_gx = fits.open("/data/gleam_x/processing/GLEAMX-DRII/GLEAMX_DRII_filtered_sedfit_quality.vot")
gx = hdu_gx[1].data
...
#        ax.plot(mosaic_sigmas, snapshot_ratios, zorder=20, ls="-", lw=1, label=f"{n}")
    ...
    ax.axhline(0.98, color="k", lw=0.5, ls="--")
...
#    ax.legend()
    fig.savefig("GLEAM-X_GLEAMcorr_nofit_ratio_hexbins_drii.pdf",bbox_inches="tight",dpi=1000,overwrite=True)
...
if makeS is True:
    ...
    fig.savefig("GLEAM-X_GLEAM_S200_comparison_drii.pdf", bbox_inches="tight",overwrite=True)
...
#    ax.yaxis.set_major_formatter(FuncFormatter(lambda y, _: '{:g}'.format(y)))
    ...
    fig.savefig("GLEAM-X_GLEAM_alpha_comparison_drii.pdf", bbox_inches="tight",overwrite=True)
