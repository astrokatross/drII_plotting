...
def do_everything(cat_neg_path, cat_pos_path, pos_ra_col='ra', pos_dec_col='dec', output_positive=None):
    ...
    idx1 = np.intersect1d(idx, np.where(cat_neg["ra"] <= ramin))
    idx2 = np.intersect1d(idx, np.where(cat_neg["ra"] >= ramax ))
    idx = np.union1d(idx1, idx2)

    cat_neg = cat_neg[idx]
    print(len(cat_neg))

    idx = np.where(cat_full[dec_col] < decmax)
    # idx = np.intersect1d(idx, np.where(cat_full[dec_col] > decmin))
    idx1 = np.intersect1d(idx, np.where(cat_full[ra_col] <= ramin))
    idx2 = np.intersect1d(idx, np.where(cat_full[ra_col] >= ramax ))
    idx = np.union1d(idx1, idx2)
    ...
    for i in range(nneg):
        ...
        try:
            ...
            print("Unable to find a corresponding bright source near a negative... ")
            bad_index.append(i)
    ...
    if output_positive is not None:
        ...
        fits.writeto(output_positive, cat_full[good_pos], header = cat_fullhd,overwrite=True)
...
def read_and_plot(cat_neg_path):
    ...
    # cat_ned[good_neg]
    cat_neg_good_neg = fits.getdata(f"{args.datadir}XG_170-231MHz_psf_comp_negative_dafix_comp_filtered.fits", 1) 
    print(len(cat_neg))

    # cat_full[good_pos]
    cat_full_good_pos = fits.getdata(f"{args.output_positive}", 1) 

    # cat_neg[combined_good_neg]
    cat_neg_comb_good_neg = fits.getdata(f"{args.datadir}XG_170-231MHz_psf_comp_negative_dafix_comp_filtered_by_both_filters.fits", 1)

    fig = plt.figure(figsize = (8*cm, 8*cm))
    ax = fig.add_subplot(111)
    hxy = ax.hist(cat_neg['int_flux']/cat_neg['local_rms'], bins = 30, alpha = .5, color = 'blue', label = 'No Filtering')
    hxy2 = ax.hist(cat_neg_good_neg['int_flux']/cat_neg_good_neg['local_rms'], bins = hxy[1], alpha = 0.8, color = 'r', label = 'Filtered')
    plt.axis([-25, -4, 0., 1000.])
    ax.legend(loc = 2)#, prop = {'size':13})
    ...
    # For negative sources, we can apply that filter:
    hsnr_neg_single = np.histogram(cat_neg_good_neg['int_flux']*-1/cat_neg_good_neg['local_rms'], bins = bin_edge)
    # and also the filter of removing the ones that lie near other sources
    hsnr_neg_double = np.histogram(cat_neg_comb_good_neg['int_flux']*-1/cat_neg_comb_good_neg['local_rms'], bins = bin_edge)
    ...
    ax.legend(loc='lower right')
    fig.tight_layout()
    fig.savefig(f"{args.savedir}reliability.pdf")
    fig.savefig(f"{args.savedir}reliability.png", transparent=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datadir', type=str, default="data/",help="directory where everything is stored")
    parser.add_argument("--savedir", type=str, default="plots/", help="Directory to save the plots")
