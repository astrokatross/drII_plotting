...
GLEAMX_PEA = [f'peak_flux_{c}' for c in CHANSN]
GLEAMX_A = [f'a_{c}' for c in CHANSN]
GLEAMX_ERRA = [f'err_a_{c}' for c in CHANSN]
GLEAMX_B = [f'b_{c}' for c in CHANSN]
GLEAMX_ERRB = [f'err_b_{c}' for c in CHANSN]
...
def get_freq_flux_err(src_row, freqs=GLEAMX_FREQS, fluxes=GLEAMX_INT, errs = GLEAMX_ERR, apply_mask = True, internal_scale=0.02):
    
    freq = GLEAMX_FREQS
    int_flux = np.array([src_row[i] for i in GLEAMX_INT])
    err_flux = np.array([src_row[i] for i in GLEAMX_ERR])
    local_rms = np.array([src_row[i] for i in GLEAMX_RMS])
    a = np.array([src_row[i] for i in GLEAMX_A])
    err_a = np.array([src_row[i] for i in GLEAMX_ERRA])
    b = np.array([src_row[i] for i in GLEAMX_B])
    err_b = np.array([src_row[i] for i in GLEAMX_ERRB])
    peak_flux = np.array([src_row[i] for i in GLEAMX_PEA])
    # err_peak_flux = np.array([row[i] for i in GLEAMX_ERRPEA])
    
    # Gleam-x internal flux error
    # 
    
    # err_peak_flux = np.sqrt(local_rms**2 + (peak_flux*internal_scale)**2)
    # err_flux = np.sqrt((err_a/a)**2 + (err_b/b)**2 + (err_peak_flux/peak_flux)**2)*int_flux 

    err_flux = np.sqrt( local_rms ** 2 + (int_flux * internal_scale)**2)
